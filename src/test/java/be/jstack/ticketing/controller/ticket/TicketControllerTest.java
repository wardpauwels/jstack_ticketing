...
public class TicketControllerTest {
    ...
    public void addNewTicketTest() {
        ...
        when(userDetailsService.getLoggedInUser()).thenReturn(new User("Bert", "Bert"));

        Ticket addedTicket = ticketController.createNewTicket("{'username':'Bert','message':'Mijn lampen werken niet.'}");
    ...
    public void answerOnTicketWithIdTest() {
        ...
        when(userDetailsService.getLoggedInUser()).thenReturn(new User("Flor", "Flor"));

        Ticket ticket = ticketController.answerOnTicketWithId(String.valueOf(firstTicket.getId()), "{'answer':'Steek de stekker in.'}");
        ...
        verify(userDetailsService, times(1)).getLoggedInUser();
        verifyNoMoreInteractions(ticketService);
    }
    ...
    public void ticketNotSolvedTest() {
        when(ticketService.findTicket(secondTicket.getId())).thenReturn(secondTicket);
        when(associationService.getTopAssociationFromTicket(secondTicket)).thenReturn(firstAssociation);
        Ticket ticket = ticketController.ticketNotSolved(String.valueOf(secondTicket.getId()), "{'comment':'De stekker steekt in.'}");
